...
export function Members({ households, locations, onMenuClick }: Omit<MembersProps, 'members' | 'onCreateMember' | 'onUpdateMember' | 'onDeleteMember'>) {
  ...
  const handleSave = async (memberData: Partial<FamilyMember>, extraMembers?: HouseholdMemberRow[]) => {
    try {
      ...
      const { household_name, ...memberDataWithoutImage } = payload as any;
      let savedMember: any;
      let householdId = memberDataWithoutImage.household_id;
      if (editingMember) {
        savedMember = await supabaseHelpers.updateFamilyMember(editingMember.id, memberDataWithoutImage as any);
      } else {
      ...
      // If leader was just created and there are extra household members, batch-save them
      if (!editingMember && extraMembers && extraMembers.length > 0 && savedMember) {
        const membershipDateStr = (memberData as any).membership_date?.toISOString?.().split('T')[0] ?? 
                                  (memberData.membership_date instanceof Date ? memberData.membership_date.toISOString().split('T')[0] : String(memberData.membership_date || '').split('T')[0]) ?? 
                                  new Date().toISOString().split('T')[0];
        const memberRows = extraMembers
          ...
          .map(row => ({
            ...
            age: row.birthdate ? Math.floor((Date.now() - new Date(row.birthdate).getTime()) / (365.25 * 24 * 60 * 60 * 1000)) : undefined,
            household_id: savedMember.household_id || householdId || null,
            lgu: savedMember.lgu || memberData.lgu || null,
            barangay: savedMember.barangay || memberData.barangay || null,
            purok: savedMember.purok || memberData.purok || null,
            purok_id: savedMember.purok_id || memberData.purok_id || null,
            sector: row.sector || 'General',
            year_level: row.sector?.includes('Student') ? row.year_level : undefined,
        ...
        for (const row of memberRows) {
          await supabaseHelpers.createFamilyMember(row as any);
        }
      }
  ...
  // Unique LGU and Barangay options from locations prop
  const uniqueLgus = [...new Set(locations.map(l => l.lgu).filter(Boolean))].sort();
  ...
  // Reset barangay when LGU changes
      ...
      <div className="p-4 lg:p-6">
        ...
        <div className="flex flex-col gap-3 mb-6">
          ...
          <div className="flex flex-col sm:flex-row gap-2 flex-wrap">
            <select value={filterLgu} onChange={(e) => handleLguChange(e.target.value)} className="px-3 py-3 lg:py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-teal-500 focus:border-teal-500 text-base lg:text-sm">
              <option value="">All LGUs</option>
              {uniqueLgus.map(lgu => (<option key={lgu} value={lgu}>{lgu}</option>))}
            </select>
            <select value={filterBarangay} onChange={(e) => handleBarangayChange(e.target.value)} className="px-3 py-3 lg:py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-teal-500 focus:border-teal-500 text-base lg:text-sm">
              <option value="">All Barangays</option>
              {uniqueBarangays.map(barangay => (<option key={barangay} value={barangay}>{barangay}</option>))}
            </select>
