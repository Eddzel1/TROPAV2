...
export function SectorReport({ members }: SectorReportProps) {
  const sectorStats = useMemo(() => {
    const stats = members.reduce((acc, member) => {
      ...
      sectors.forEach(sector => {
        if (!acc[sector]) acc[sector] = { total: 0, cooperativeMembers: 0, voters: 0, householdLeaders: 0, ageSum: 0, ageCount: 0, households: new Set<string>() };
        const stat = acc[sector];
  ...
  const ageDistributionBySector = useMemo(() => {
    ...
    members.forEach(member => {
      ...
      const sectors = member.sector ? member.sector.split(',').map(s => s.trim()).filter(Boolean) : ['General'];
      sectors.forEach(sector => {
        if (!distribution[sector]) distribution[sector] = {};
        distribution[sector][ageGroup] = (distribution[sector][ageGroup] || 0) + 1;
      });
    }); return distribution;
  }, [members]);
